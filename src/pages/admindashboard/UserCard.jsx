...
const UserCard = ({ user, users, setUsers }) => {
    ...
    const { _id, name, email, isadmin, ismember} = user;

    const handleUpdateAsAdmin = _id => {
        const updatedUser = { isadmin: true };

        fetch(`https://m12a-forum-server.vercel.app/user/${_id}`, {
            method: 'PUT',
            headers: {
            ...
            // Reload user data
            const reloadedUsers = users.map(u => (u._id === _id ? { ...u, isadmin: true } : u));
            setUsers(reloadedUsers);
            
    }

    const handleUpdateAsNonAdmin = _id => {
        const updatedUser = { isadmin: false };
        ...
        fetch(`https://m12a-forum-server.vercel.app/user/${_id}`, {
            ...
            .then(data => {
                ...
                if (data.modifiedCount > 0) {
                    Swal.fire({
                        title: 'Success!',
                        text: 'User Updated as Non Admin Successfully',
                        icon: 'success',
                        confirmButtonText: 'Cool'
                    })
                }
            ...
            // }, 2000);
            // Reload user data
            const reloadedUsers = users.map(u => (u._id === _id ? { ...u, isadmin: false } : u));
            setUsers(reloadedUsers);
    }
        ...
        <>
            <tr className="hover:bg-gray-50">
                ...
                <div className="text-sm">
                    ...
                    {/* <div className="text-gray-400">{author_email}</div> */}
                </div>
                </th>
  ...
  {isadmin === true ? <button className="inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-1 text-xs font-semibold text-green-600" onClick={() => handleUpdateAsNonAdmin(_id)}>Admin</button> : <button className="inline-flex items-center gap-1 rounded-full bg-red-50 px-2 py-1 text-xs font-semibold text-red-600" onClick={() => handleUpdateAsAdmin(_id)}>Make Admin</button>}
